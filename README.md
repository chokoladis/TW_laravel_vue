...
2. Административная часть (требуется аутентификация):
   ...
   > ■ Поля: Название (input).
   > <br> ■ Категория (select, загружается из GET /api/categories).
   > <br> ■ Описание (textarea), Цена (input type number).
   > <br> ■ Валидация на фронтенде (можно простую, required, min для цены).
   > <br> ■ Отправка данных на соответствующий API эндпоинт.
   > <br> ■ После успешного сохранения — редирект на список товаров администратора.
